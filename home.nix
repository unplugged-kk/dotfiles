...

{
  ...
  # gws-axi / gitsheets-axi / pg-axi / cyber-mux / graft are npm-global
  # (installed under npm's configured prefix, ~/.local per `npm config get
  # prefix`); docker-axi / kubernetes-axi install from GitHub (not yet on npm);
  # specops is a skill pack (installed by bootstrap Step 13, not an npm global);
  # treehouse / no-mistakes are Go binaries in ~/.local/bin with their own
  # `update` subcommand. None of these are Nix-packaged, so home-manager can't
  # pin/build them - this just makes sure every `./rebuild.sh` also brings them
  # current, instead of them silently drifting stale between manual runs.
  # Best-effort: a flaky network here must never fail the whole home-manager
  # switch.
  home.activation.updateAgentCliTools = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    if command -v npm >/dev/null 2>&1; then
      run npm install -g \
        ...
        gitsheets-axi@latest pg-axi@latest cyber-mux@latest graft@latest \
